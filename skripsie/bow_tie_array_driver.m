    ...
    if  m ~= n
      ...
      Rmn = sqrt((edge_m_X - edge_n_X)^2 + (edge_m_Y - edge_n_Y)^2);
       ...
       %if Rmn >= (0.5*lambda)
          ...
          plot(ax1,newFrequency,new_real1,'-xb',fq,vq,'-');  
          ...
          %Apply interpolation
          plot(ax2,newFrequency,new_imag1,'-xb',fq,vr,'-');  %(Spline interp1)
         ...
         Zinterp2 = Zinterp1 .* exp(-1i*((2*pi)./lambda')*Rmn);
         error = (norm(matrix_Z  - Zinterp2)/(norm(matrix_Z )))* 100;

        % legends{m,n} = sprintf('m,n = %d,%d', m,n);

        % now add field called Interpolated
         [zMatrices(:).interpolated] = Zinterp2;
         fieldnames(zMatrices);
